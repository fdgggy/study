...
public class RoomGenerator : MonoBehaviour
{
    ...
    public void SetupRoom(Room newRoom, Vector3 roomPosition)
    {
        ...
        switch (newRoom.doorNumer)
        {
            ...
            case 3:
                ...
                else if (newRoom.roomLeft && newRoom.roomRight && newRoom.roomDown)
                {
                    Instantiate(wallType.tripleLRB, roomPosition, Quaternion.identity);
                }
                else if (newRoom.roomDown && newRoom.roomUp && newRoom.roomRight)
                {
                    Instantiate(wallType.tripleURB, roomPosition, Quaternion.identity);
                }
                else if (newRoom.roomLeft && newRoom.roomUp && newRoom.roomDown)
                {
                    Instantiate(wallType.tripleLUB, roomPosition, Quaternion.identity);
                }
                break;
            case 4:
                if (newRoom.roomLeft && newRoom.roomUp && newRoom.roomRight && newRoom.roomLeft)
                {
    ...
    public void FindEndRoom()
    {
        ...
        {
            if (farRooms[i].GetComponent<Room>().doorNumer == 1)
            {
        ...
        {
            if (lessFarRooms[i].GetComponent<Room>().doorNumer == 1)
            {
